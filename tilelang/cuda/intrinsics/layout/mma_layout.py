...
def metal_ct_store_32x16_to_16x32_layout(thread_id, local_id):
    """Metal M5 cooperative-tensor store lane/elem layout.

    Mirrors the (__lane, __qid, __base_row, __base_col) decode emitted by
    ``EmitCooperativeTensorLanePreambleIfNeeded`` in ``codegen_metal.cc``.
