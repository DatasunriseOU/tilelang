    ...
    def transform_function(self, func: tir.PrimFunc, mod: tvm.IRModule, ctx: tvm.transform.PassContext) -> tir.PrimFunc:
        ...
        def _mutate(node):
            ...
            if intrinsic is None:
                raise ValueError(f"extern_intrinsic '{intrinsic_name}' is not registered; register it before lowering the TileLang kernel.")
            body = intrinsic.bodies.get(self.target_name)
            if body is None:
                targets = ", ".join(sorted(intrinsic.bodies))
                raise ValueError(
                    f"extern_intrinsic '{intrinsic_name}' has no body for target '{self.target_name}'. Registered targets: [{targets}]."
                )
        ...
        if bodies:
            ...
            new_body = tir.AttrStmt(
                node=tir.StringImm("pragma_import_c"), attr_key="pragma_import_c", value=tir.StringImm(combined_body), body=new_body
            )
