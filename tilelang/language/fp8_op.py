...
final post-dot multiply by ``A_scale * B_scale``; the dispatching and
codegen path is identical. Mirroring the audiohacking scalar K-loop
therefore reduces to: take the scalar dequantize-and-dot body that Agent
E already validated and apply the broadcast scale once to the completed
dot product — which is exactly what this macro emits.
...
def _is_metal_target(target: Optional[Target]) -> bool:
    """Return True when the caller explicitly requests Metal lowering."""
    if target is None:
        return False
...
def _fp8_scaled_matmul_macro(A_fp8, A_scale, B_fp8, B_scale, C_local):
    ...
    audiohacking kernel's threadgroup-tiling pattern. The scale multiply
    is deliberately outside the K-loop: it applies once to the completed
    dot product while preserving ``C_local += ...`` read-modify-write
    semantics.
    """
    ...
    # Accumulate the raw dot product first, then scale the completed dot
    # once. This is the important audiohacking fast-path property: scale
    # work is O(MN), not O(MNK), while C_local remains read-modify-write.
...
@T.macro
def _fp8_scaled_matmul_m1_vecmat_metal_macro(A_fp8, A_scale, B_fp8, B_scale, C_local):
    """Metal M=1 vecmat path: 32 lanes reduce K with ``simd_sum``.

    This is only dispatched for ``transpose_B=True`` and ``A.shape[0] == 1``.
    B is row-major ``(N, K)``. The inner lane-private dot remains unscaled;
    scales are applied once after the SIMD-group reduction.
    """
    M_dim, K_dim = A_fp8.shape
    N_dim, K_dim_b = B_fp8.shape
    sa_size = A_scale.shape[0]
...
def fp8_scaled_matmul(
    ...
    * **CPU / fallback** — same scalar TIR; ``T.cast(fp8, fp32)`` lowers
    ...
    Args:
        ...
        target: Optional lowering target. When explicitly set to Metal and
            ``transpose_B=True`` with ``M == 1``, dispatches to the vecmat
            specialization that reduces K across a 32-lane SIMD group with
            ``simd_sum``.
    ...
    if transpose_B and _is_metal_target(target) and _shape_extent(A_fp8, 0) == 1:
        return _fp8_scaled_matmul_m1_vecmat_metal_macro(A_fp8, A_scale, B_fp8, B_scale, C_out)
    if transpose_B:
