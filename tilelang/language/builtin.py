...
def cooperative_tensor_fill(data, idx, value, rows: int, cols: int):
    """Metal M5 cooperative tensor fill builtin (PR tile-ai/tilelang#2252).

    Emits one ``tl.cooperative_tensor_fill`` call which the Metal codegen
    expands into a per-thread loop over the lane-private tile slice.  Only
    meaningful on M5+ silicon under MSL4.
    """
    return evaluate(
        tirx.call_intrin(
            "handle",
...
def cooperative_tensor_load(
    ...
    """Metal M5 cooperative tensor load (PR tile-ai/tilelang#2252)."""
    return evaluate(
        tirx.call_intrin(
            "handle",
...
def cooperative_tensor_store(
    ...
    """Metal M5 cooperative tensor store (PR tile-ai/tilelang#2252)."""
    return evaluate(
        tirx.call_intrin(
            "handle",
