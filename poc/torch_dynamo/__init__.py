...
# Wave-4 #09 fix: ``_validate_graph`` and ``UnsupportedFXOpError`` were moved
# to ``_graph_validation`` so that ``aot_autograd_glue`` can import them via a
# real submodule path (``from ._graph_validation import validate_graph``)
# instead of the broken ``from . import _validate_graph`` which used to
# resolve to the package object. We re-export them here for back-compat.
from ._graph_validation import UnsupportedFXOpError, validate_graph as _validate_graph
