"""dot_reduce_atomic_trans_b: transposed-B dot + reduce + atomic smoke.

This mirrors ``dot_reduce_atomic`` but stores B as ``(N, K)`` and transposes
the tile at dot time, matching the Path C transposed-B patterns used by the
cppmega integration tests.
"""
...
if triton is not None:
    ...
    @triton.jit
    def _dot_reduce_atomic_trans_b_kernel(
        a_ptr,
        b_ptr,
        acc_ptr,
        ...
        b = tl.load(b_ptr + offs_n[:, None] * BLOCK_K + offs_k[None, :])
    ...
    TRITON_KERNEL: Callable[..., Any] = _dot_reduce_atomic_trans_b_kernel
...
def numpy_reference(args: list[np.ndarray]) -> np.ndarray:
    a, b, _acc = args
    c = a.astype(np.float32) @ b.astype(np.float32).T
