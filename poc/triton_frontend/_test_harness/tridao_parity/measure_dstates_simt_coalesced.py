"""ITERATION 7 -- SIMT-coalesced de-risk measurement.

Builds the routed _chunk_scan_bwd_dstates two ways and measures executed ms:
  OFF  = prologue_opt=False (baseline serial path, scalar LDG)
  OPT  = prologue_opt=True, async_loads=False -> routed CopyNode lowers via the
         SIMT cooperative-vectorized path (disable_tma=True => Copy::Lower takes
         the SIMT branch, NOT LowerBulk/TMA). The SIMT loop vectorizer emits
         coalesced LDG.E.128 over the contiguous axis. async_loads=False keeps
         the masked routed copy OUT of the cp.async pipeline (the mask epilogue
         + copy overlap is a SEPARATE pipelining blocker); coalescing itself is
         a LOAD property delivered by the vectorizer independent of pipelining.

RULE #1: this is the EXECUTED coalesced-load measurement. No TMA (which faults
on sm_121). Parity must stay bit-correct.
"""
import sys
...
def build(prologue_opt, async_loads):
...
k_off = build(False, False)
print("building OPT (prologue_opt=True, async_loads=False -> SIMT coalesced)...", flush=True)
k_opt = build(True, False)
print("OFF nparams=%d OPT nparams=%d" % (len(k_off.params), len(k_opt.params)), flush=True)

# dump the OPT cuda source for codegen verification
src_opt = k_opt.get_kernel_source() if hasattr(k_opt, "get_kernel_source") else None
if src_opt:
    open("/tmp/ttir7/dstates_opt_simt.cu", "w").write(src_opt)
    has_tma = ("tma_load" in src_opt) or ("tensorMap" in src_opt) or ("cp.async.bulk.tensor" in src_opt)
...
print("=== SIMT-COALESCED CUDA-EVENT (N=%d/rep x4, median-of-medians) ===" % N)
print("OFF_MS=%.4f" % off)
print("OPT_MS=%.4f" % opt)
