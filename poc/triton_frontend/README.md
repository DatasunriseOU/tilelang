...
+------------------+        +------------------+        +-------------------+
   ...
   (Triton 3.6: capture       (83 entries in OP_TABLE)       LowerTileOp, ...)
    via make_ir(target,                |
    options, codegen,                  v
    module_map, ctx))         +----------------------+
...
| `op_mapping.py`                       | Dispatch table: TTIR op name -> emitter. **83 entries** post-Wave-3.      | section 5.1   |
...
* **Op coverage**: 83 entries in `OP_TABLE` covering memory, arith,
