...
+------------------+        +------------------+        +-------------------+
   ...
   (Triton 3.6: capture       (101 entries in OP_TABLE)      LowerTileOp, ...)
    via make_ir(target,                |
    options, codegen,                  v
    module_map, ctx))         +----------------------+
...
| `op_mapping.py`                       | Dispatch table: TTIR op name -> emitter. **101 entries** (post FLA Path D + bitwise/logical + maxnum/minnum + index_cast enable). | section 5.1   |
...
* **Op coverage**: 101 entries in `OP_TABLE` covering memory, arith,
