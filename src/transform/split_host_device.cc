...
public:
  ...
  void SetNonRestrictParams(Optional<Array<tirx::Var>> params) {
...
private:
  ...
  tirx::Stmt SplitDeviceFunc(tirx::Stmt body, tvm::Target device_target) {
    ...
    for (tirx::Buffer buf : buffers_to_declare) {
      // CPPMEGA: DeclBuffer is body-less in apache; sequence it before body.
      body = tirx::SeqStmt::Flatten(
          tvm::ffi::Array<tirx::Stmt>{tirx::DeclBuffer(buf), body});
    }
    ...
    if (can_propagate_errors) {
      ...
      tirx::AssertStmt assert_success(
          kernel_error_code == success, tirx::StringImm("RuntimeError"),
          ffi::Array<tirx::StringImm>{
              tirx::StringImm("Error executing compute kernel")});
      // CPPMEGA: Avoid emitting a vendored `tilelang::tl_tir::LetStmt` here:
      // this function returns into the host body which is then handed to
      // `tirx::transform::ConvertSSA()` (see line ~469). ConvertSSA is an
      // apache `StmtExprMutator` whose `StmtFunctor` vtable does not know
      // how to dispatch on `tilelang.LetStmt` (NodeFunctor crash at
      // `node_functor.h:97`). Use the apache equivalent
      // `SeqStmt({Bind(var, value), body})` directly.
      tirx::Bind let_bind(kernel_error_code, kernel_call);
      return tirx::SeqStmt({let_bind, assert_success});
