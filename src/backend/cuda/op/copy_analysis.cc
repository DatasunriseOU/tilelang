...
bool Z3ProveStrideAligned16(arith::Analyzer *analyzer,
                            const PrimExpr &addr_bytes,
                            const PrimExpr &stride_bytes, Target target) {
  ...
  try {
    ...
    if (target.defined()) {
      auto attr = target->GetAttr<Integer>("max_addr_bits");
      if (attr.defined()) {
        addr_bits = attr.value()->value;
      }
    }
    int64_t addr_envelope = (addr_bits >= 63)
    ...
    ::tilelang::tlz3::ConstraintScope scope(z3, addr_constraint &&
                                                    stride_constraint);
    PrimExpr goal = (FloorMod(addr_bytes, IntImm(DataType::Int(64), 16)) ==
...
bool CheckGlobalStrides(const Buffer &buffer, arith::Analyzer *analyzer,
                        bool emit_diagnostics, Target target) {
  ...
  for (size_t i = 0; i + 1 < strides.size(); ++i) {
    ...
    PrimExpr aligned = FloorMod(stride_bytes, IntImm(DataType::Int(64), 16)) ==
                       IntImm(DataType::Int(64), 0);
    ...
    if (!analyzer->CanProve(aligned, arith::ProofStrength::kSymbolicBound)) {
      ...
      bool z3_legality_enabled =
          ::tilelang::tlz3::GetProofHookStatus(
              pass_ctx, ::tilelang::tlz3::ProofHookKind::kAsyncEligibility,
              kTMALegalityZ3)
              .enabled;
      if (z3_legality_enabled) {
        int64_t elem_bytes = static_cast<int64_t>(buffer->dtype.bytes()) *
                             static_cast<int64_t>(buffer->dtype.lanes());
        PrimExpr addr_bytes = cast(DataType::Int(64), buffer->elem_offset) *
                              IntImm(DataType::Int(64), elem_bytes);
        if (!Z3ProveStrideAligned16(analyzer, addr_bytes, stride_bytes,
                                    target)) {
          if (emit_diagnostics) {
            DLOG(WARNING) << "TMA bulk copy alignment unprovable (Z3): addr="
...
bool CheckBulkLoad(const CopyNode &op, Target target, arith::Analyzer *analyzer,
  ...
  return CheckGlobalStrides(op.src, analyzer, emit_diagnostics, target);
}
...
bool CheckBulkStore(const CopyNode &op, Target target,
  ...
  return CheckGlobalStrides(op.dst, analyzer, emit_diagnostics, target);
}
