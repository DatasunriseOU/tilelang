...
struct Fill {
  static Stmt Lower(const FillNode &op, const LowerArgs &T,
    ...
    // Metal M5 cooperative tensor fill: emit one tl::cooperative_tensor_fill
    // call per 16x32 micro tile.  See PR tile-ai/tilelang#2252.
    if (IsCooperativeTensorBuffer(op.dst)) {
      ...
      ICHECK(region_elements % kTileElems == 0)
      ...
      for (int i = 0; i < num_tiles; i++) {
        stmts.push_back(
            Evaluate(Call(DataType::Handle(), tl::cooperative_tensor_fill(),
