...
def _make_vecmat_m1_kernel(
    ...
    *,
    ...
    @T.prim_func
    def fp8_scaled_vecmat_kernel(
        A_fp8: T.Tensor((1, _VM_K), _VM_A_DTYPE),
        A_scale: T.Tensor((_VM_SA,), "float32"),
        B_fp8: T.Tensor((_VM_N, _VM_K), _VM_B_DTYPE),
        B_scale: T.Tensor((_VM_SB,), "float32"),
        C: T.Tensor((1, _VM_N), "float32"),
    ):
        with T.Kernel(T.ceildiv(_VM_N, _VM_BN), threads=32) as bx:
            A_shared = T.alloc_shared((1, _VM_BK), _VM_A_DTYPE, scope="shared")
            B_shared = T.alloc_shared((_VM_BN, _VM_BK), _VM_B_DTYPE, scope="shared")
            C_local = T.alloc_fragment((1, _VM_BN), "float32")
            T.clear(C_local)
