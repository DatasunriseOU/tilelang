...
_SCALE_CU = r"""
__device__ void vector_scale_2(const float* in_vec, float* out_vec) {
    #pragma unroll
    for (int i = 0; i < 16; ++i) {
...
def run_cuda_extern_intrinsic():
    # 1. Define a @tl.extern_intrinsic with a cuda body
    extern_intrinsic(
        name="vector_scale_2",
        signature=lambda: (
    ...
    # 3. Build the kernel for the "cuda" target
...
def test_cuda_extern_intrinsic():
    run_cuda_extern_intrinsic()
