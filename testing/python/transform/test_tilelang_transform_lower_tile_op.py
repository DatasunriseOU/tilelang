...
def test_lower_tile_op_respects_no_implicit_commit_wait_annotation():
    ...
    def before(
        ...
        T.async_copy(
            A[0:16],
            S,
            annotations={"no_implicit_async_commit_wait": T.bool(True)},
        )
        B[tx] = S[tx]
...
def test_lower_tile_op_emits_commit_for_explicit_async_copy():
    target = tvm.target.Target({"kind": "cuda", "arch": "sm_80"})
